...
## Ik moet goed gaan nadenken over het resolutie vraagstuk: als ik nx omhoog doe gaat het fout bij de calcificatie in het model. Waarom? is die niet goed gekoppeld aan daadwerkelijke hoeveelheid beschikbare bouwstenen? Raken de 'volumes' van de boxen vol? Ik moet hier met Karline over praten, maar zal iig:
...
Area = 5.00e6 #7e6#2.6e7   # global coastal surface area km2, from Milliman 1993
Qriver =  7.41e3#1e4  #1*40e3/sec_to_year # river discharge flow rate [km3/s]
Eh =  0#1.106797e-04 #0#1e-2# this value gives an effect#250*10**(-9) #from the Soetaert and Herman paper, estimate for Westerschelde. How to enforce constant horizontal dispersion. Equal in size in seaward and landward direction?  # horizontal dispersion [km3/s] 


# Bathymetry
# CODE BELOW ASSUMES 3 DEPTH RANGES WITH 3 DISTINCT ENVIRONMENTS
# --> simulated environment for testing calcification: warm sea, surface boxes 5 m deep. Bottom boxes divided into three steps: first part bottom boxes 10 m deep, next part 50 m deep, last part 140 m
d_surf = 5  # in m
...
#dividing bottom boxes into three temperature regions
Ts = 26#18 #26 # degrees Celcius
Tbs = 25#17 #25 
Tbm = 20#12 #20
Tbd = 15#7 #15
...
## Calculating the settings for each box, based on prescribed boxnumer
# --> determining the amount of boxes for each depth range
# --> --> assumption coral reefs only 1/50th of the total margin area.
# --> --> --> For this nx should be multiple of 5. the first 1/5th of the boxes will be coralreef depth, and in the calcification code itself, we further multiply by 1/10 to arrive at an appropriate coral cover.


Box_settings <- function(nx, p){
  
  #Salinity
  S = cbind(c(5, seq(from=25, to=35, length.out=nx + 1)), seq(from=30, to=35, length.out=nx +2)) # salinity matrix, first colum representing surface boxes, second column bottom boxes
#S = cbind(seq(from=5, to=35, length.out=nx + 2), seq(from=30, to=35, length.out=nx +2))
  ...
  #Volume (recalculate to km3, this unit is needed for the calculation of flows in hydrodynamics-script)
...
# #dividing bottom boxes into three temperature regions
# Ts = 26 # degrees Celcius
# Tbs = 25 
# Tbm = 20
# Tbd = 15
# 
# #Light settings
# E0 = 10 /(24*60*60) # surface light, mol quanta/m2/s
# kpar = 0.5 # coastal waters
# 
# Envparms <- c(Ts, Tbs, Tbm, Tbd, E0, kpar)
# 
# ## Calculating the settings for each box, based on prescribed boxnumer
...
#   with(as.list(p),{
#   Temp_surf <- c(rep(Ts, times =nx))
#   Temp_bottom <- c(rep(Tbs, times =nx/5), rep(Tbm, times =2*(nx/5)), rep(Tbd, times =2*(nx/5)))
#   Temp <- c(Temp_surf, Temp_bottom)
#   
#   #Light, average over box depth    
#   z <- seq(0, 150, by=1)   # depth in meters
#   #   for (i in z ){
#   # E <- E0 * exp(-kpar *z)
#   # E_s <- c(rep(mean(E[0: (d_surf)]), times =nx)) 
#   # E_bs <- c(rep(mean(E[((d_surf+1): (d_bottom_s+d_surf))]), times =nx/5))
...
#   E <- E0 * exp(-kpar *z)
#   E_s <- c(rep(E[d_surf], times =nx)) 
#   E_bs <- c(rep(E[(d_bottom_s+d_surf)], times =nx/5))
#   E_bm <- c(rep(E[(d_bottom_m+d_surf)], times =2*(nx/5)))
#   E_bd <- c(rep(E[(d_bottom_d+d_surf)], times =2*(nx/5)))
#   Ebox <-c(E_s, E_bs, E_bm, E_bd) # mol quanta/m2/s
#     }
#   
#   #Volume (recalculate to km3, this unit is needed for the calculation of flows in hydrodynamics-script)
#   Vsurf <- c(rep(d_surf/1000*(Area/nx)))
#   Vbottom <-c(rep(d_bottom_s/1000*(Area/nx), times = nx/5), rep(d_bottom_m/1000*(Area/nx), times = 2*(nx/5)),rep(d_bottom_d/1000*(Area/nx), times = 2*(nx/5)) )
#   BoxVolume <- cbind(Vsurf, Vbottom)
#   
#   list('Temp'=Temp, 'Ebox' = Ebox, 'BoxVolume'= BoxVolume, 'S'= S)
#   
#   })
#}
